...
├── siem-desktop/                  # Tauri desktop-приложение (замена siem-operator)
...
| Файл | Назначение |
|------|------------|
| `clickhouse_password.txt` | Пароль пользователя `siem` в ClickHouse; совпадает с `CLICKHOUSE_PASSWORD` в compose |
| `minio_secret_key.txt` | Пароль root в MinIO Console (**логин** `siemadmin`) |
| `grafana_admin_password.txt` | Пароль администратора Grafana (**логин** `admin`) |
| `smtp_password.txt`, `slack_webhook_url.txt`, `pagerduty_key.txt` | Alertmanager (можно заглушки для чисто локальной проверки) |

Пример (Linux/macOS/Git Bash; в PowerShell используйте `scripts/init-secrets.ps1` — см. ниже):
...
|--------|-----|-----------------------------|
| Grafana | http://localhost:3000 | `admin` + пароль из `grafana_admin_password.txt` |
...
Сервис **`intel-connector`** периодически загружает IoC в **`siem.threat_intel`** (ClickHouse) и при `INTEL_SYNC_REDIS=1` зеркалирует наборы в **Redis** (`siem:intel:ipv4` и др.). Для матча на лету в нормализованном событии задайте у **`siem-parser`** переменную **`SIEM__INTEL__REDIS_URL=redis://:password@redis:6379/0`** (Redis требует аутентификацию — пароль совпадает с `REDIS_PASSWORD` в compose; см. `deploy/docker/docker-compose.yml`).
